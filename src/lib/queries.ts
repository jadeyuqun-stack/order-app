...
export function placeOrder(dailyOrderId: string, employeeId: string, dishName: string, price: number, quantity: number) {
  db.prepare('INSERT INTO orders (id, daily_order_id, employee_id, employee_name, dish_name, price, quantity) VALUES (?, ?, ?, ?, ?, ?, ?)').run(uuidv4(), dailyOrderId, employeeId, '', dishName, price, quantity);
}
...
export function getAllOrdersForDate(date: string, sortBy: string = 'name') {
...
export function updateOrder(id: string, quantity: number) {
